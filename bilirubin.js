...
document.addEventListener('DOMContentLoaded', function() {
    ...
    myChart = new Chart(ctx, {
        ...
        data: {
            datasets: [
                { label: 'Utskiftning', data: linesData.exchange, borderColor: 'black', borderDash: [10, 8], borderWidth: 1, pointRadius: 0 },
                ...
                { label: 'Trend', data: [], borderColor: 'black', borderDash: [10, 8], borderWidth: 2, pointRadius: 0 }
            ]
        },
        options: { 
            responsive: true, 
            plugins: {
                legend: {
                    labels: {
                        // Denne gjør at tegnforklaringen bruker 'line' istedenfor firkant
                        usePointStyle: true, 
                        pointStyle: 'line',
...
// Add infobutton function. 
const infoOverlay = document.getElementById('infoModal');
const opneKnp = document.getElementById('infobutton');
const lukkeKnp = document.getElementById('close-search');

function openInfo() {
    infoOverlay.style.display = 'flex';
}

opneKnp.addEventListener('click', () => {
    openInfo();
})
lukkeKnp.addEventListener('click', () => {
...
function calcNomogram() {
    ...
    if (v2 >= currentLimitAtT2) {
        ...
        html += `<p style="margin: 5px 0; font-size: 0.9rem;"> Viss bilirubin-verdien fortsetter å stige i samme hastighet, vil verdien krysse <strong>${riskLabel}</strong> ved ca. <strong>${Math.round(crossHour)} timers</strong> alder.</p>`;
    } else {
